...
import com.github.yun531.climate.midTerm.infra.client.dto.LandForecastResponseItem;
...
@Component
public class MidPopClientImpl implements MidPopClient {
    ...
    public MidPopClientImpl(WeatherClient client, MidTermUrls urls, ApiKey apiKey, ParseConfig parseConfig) {
        this.client = client;
        this.urls = urls;
        this.apiKey = apiKey;
    ...
    public List<MidPopDraft> requestMidPopDrafts(String regId, MidAnnounceTime announceTime) {
        ...
        LandForecastResponseItem item = JsonPath.using(parseConfig.getConfiguration())
                .parse(json)
                .read("$.response.body.items.item[0]", LandForecastResponseItem.class);
