...
public class GenerateAlertsService {
    ...
    private final int defaultLookbackHours;
    ...
    public GenerateAlertsService(
        ...
        this.defaultLookbackHours = Math.max(1, defaultLookbackHours);
    }
...
//                out.addAll(detectWarningIssued(regionId, since, cmd.warningKinds()));
    ...
    /** lookbackHours -> LocalDateTime 변환. null 이면 기본값(defaultLookbackHours) 적용 */
    private LocalDateTime normalizeSince(@Nullable Integer lookbackHours, LocalDateTime now) {
        int hours = (lookbackHours != null && lookbackHours > 0) ? lookbackHours : defaultLookbackHours;
