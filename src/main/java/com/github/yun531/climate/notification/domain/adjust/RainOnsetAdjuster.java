...
public class RainOnsetAdjuster {
    ...
    private static final Comparator<AlertEvent> BY_VALID_AT = Comparator.comparing(
            ev -> ((RainOnsetPayload) ev.payload()).effectiveTime(),
    ...
    private List<AlertEvent> filterByWindow(
        ...
        for (AlertEvent e : events) {
            ...
            LocalDateTime effectiveTime = p.effectiveTime();
            if (effectiveTime == null) continue;
            if (effectiveTime.isBefore(windowStart) || effectiveTime.isAfter(windowEnd)) continue;
